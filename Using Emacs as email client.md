![Engelbecken, Berlin](/public/IMG_20200801_205147.jpg)
...
So I get an [application specific](https://myaccount.google.com/apppasswords) password for my email account from my gmail account then I encrypt it to a file like so:
...
which will run `mu index` command after every sync, [mu](https://www.djcbsoftware.nl/code/mu/) is a program that uses Xapian to build a full text search database for the email directory, then you can use `mu` to search in your emails, `mu` comes with `Mu4e` which is the emacs interface for mu.
