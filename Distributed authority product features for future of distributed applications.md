...
It's common knowledge that most startups fail. With every startup that fails there is an application that's rendered obsolete. you can't open it or use it. The company behind it doesn't exist anymore.

All the efforts of the teams behind it from research, design, development and maintenance that got this application to this point is now a wasted effort. It's a very hard moment when you know that years of your life that you spent working on a product didn't result in something useful to other people.
...
For every feature the product team designs there are a couple of decisions that are made on behalf of the user:
...
The problem with the previous features design is that the choice is made for the user and can't be overridden by the user. That ties the feature to one source. One company that deliver the feature. so when this company closes its doors one morning this feature is now useless and the user can't do anything about it.

Instead these features can be design in a way similar to the open source applications I mentioned previously.

# Let others add more solutions

Lets take one example:
- You're designing a rating system so your application will allow rating a product and display the average rating
